...
CARGO := cargo
EXAMPLES := counter card list_keyed

.PHONY: help \
	check ci fmt fmt-check clippy test build build-examples doc doc-open \
	run-counter run-card run-list-keyed run clean
...
run-counter: ## Run the counter example
	$(CARGO) run -p counter

run-card: ## Run the card example
	$(CARGO) run -p card

run-list-keyed: ## Run the keyed list example
